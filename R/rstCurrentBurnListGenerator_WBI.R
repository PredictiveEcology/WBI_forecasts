rstCurrentBurnListGenerator_WBI <- function(pathInputs) {
  rstPath <- grepMulti(
    x = list.files(path = pathInputs, full.names = TRUE),
    patterns = c("rstCurrentBurn"),
    unwanted = c("aux", "xml")
  )
  if (length(rstPath) == 0) {
    stop(
      "No rstCurrentBurn file (.tif) found in pathInputs. ",
  ...
  rstCurrentBurnList <- lapply(rstPath, function(rPath) {
    rasName <- paste0("Year", substrBoth(
      strng = tools::file_path_sans_ext(rPath),
  ...
  ys1 <- substrBoth(
    strng = tools::file_path_sans_ext(rstPath[1]),
    howManyCharacters = 4, fromEnd = TRUE
  )
  ys2 <- substrBoth(
    strng = tools::file_path_sans_ext(rstPath[length(rstPath)]),
    howManyCharacters = 4, fromEnd = TRUE
  )
  message(paste0(
