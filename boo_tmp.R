  ...
  for (CS in c("CanESM5", "CNRM-ESM2-1")) {
    for (SS in c("SSP370", "SSP585")) {
      for (P in c("AB", "BC", "SK", "MB", "YT", "NT")) {
        fls <- list.files(path = paste0("~/GitHub/WBI_forecasts/outputs/", P, "/posthoc/"),
                          pattern = paste0("predictedCaribou_Year2091_", P, "_",
                                           CS, "_", SS, "_", RP, "_year2091.rds"),
                          recursive = TRUE)
        ...
        parameters <- list(
          birdsNWT = list(
            ...
            "nCores" = "auto",#length(bSpG), #"auto", # If not to parallelize, use 1
            "version" = birdModelVersion,
            "RCP" = RCP,
